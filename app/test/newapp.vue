...
<!-- *************第二步************** -->
      <section v-if="step === 2">
          ...
          <div class="firststep step">
              ...
              <el-checkbox-group v-model="form.basemodules">
...
<!-- **************第四步****************** -->
    ...
    <!-- ************第五步****************** -->
      <section v-if="step === 5">
      <p class="step-title">订单详情</p>
      <div class="step" id="pdf-order">

            <h3 style="padding-top: 50px">订单详情</h3>
            <p class="title">基础模块</p>
         
             <button class="base-box" v-for=" item in alllist" :key="item.id"   v-if="item.category_id === 0"  >{{item.name}}</button>
       
             <p class="title">选择的基础模块</p>
         
             <button class="base-box" v-for=" item in alllist" :key="item.id"   v-if="item.category_id === 1 && showMoudelsC(item)"  >{{item.name}}</button>

             <p class="title">选择的独立子模块</p>
         
             <button class="base-box" v-for=" item in alllist" :key="item.id"   v-if="item.category_id === 2 && showMoudelsO(item)"  >{{item.name}}</button>

        
              <p><span>订单编号：</span> {{order_no}}</p>
              <p><span>订单总金额：</span>{{totalPrices}}元</p>
  ...
  created() {
    ...
    methods: {
        ...
        fetchData() {
          ...
          console.log('请求的所有模块数据=>>>',response.data.data)
          })
        },
        commitOrder() {
          let order = this.form;
...
.step-title{
   ...
   text-align: center;
   line-height: 30px;
   background-color: rgb(53, 155, 112);
   border-radius: 5px;
}
.base-box{
  width: 30%;
